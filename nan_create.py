...
# 加载原始 npz 文件
file_path = "./raw_data/PEMS08/PEMS08.npz"
...
def add_missing_values(data, missing_percentage):
    ...
    # print('数据集 %s 的总元素数量'%key, total_elements)
    num_missing = int(total_elements * (missing_percentage / 100)) # 计算需要添加的缺失值的数量
    # print('数据集 %s 需要添加的缺失值元素数量'%key, num_missing)
    ...
    for idx in missing_indices:
        modified_data[idx] = np.nan  # 添加缺失值

    print(f"数据集 {key} 的总元素数量: {total_elements}, 添加的缺失值数量: {num_missing}")
...
# 保存处理后的文件
save_processed_data("./miss_nan/PEMS08_miss_nan_10.npz", processed_data_10)
save_processed_data("./miss_nan/PEMS08_miss_nan_30.npz", processed_data_30)

print("已生成包含缺失值的文件：PEMS04_miss_nan_10.npz 和 PEMS04_miss_nan_30.npz")
