...
# 根据配置文件中指定的执行器类型（executor）创建相应的执行器实例
...
# 根据配置文件中指定的模型类型（model）创建相应的模型实例
...
# 根据配置文件中指定的评估器类型（evaluator）创建相应的评估器实例
...
def get_logger(config, name=None):
    ...
    Returns:
        Logger: logger
    """
    log_dir = './libcity/log' # 日志目录
    if not os.path.exists(log_dir): # 日志目录不存在就创建
        os.makedirs(log_dir)
...
# 根据输入窗口和输出窗口的大小对数据进行预处理（好像没有用上该函数）
def preprocess_data(data, config):
    ...
    x, y = [], [] # 存储输入和输出
    for i in range(len(data) - input_window - output_window): # 从数据的开头到末尾生成输入输出对
        a = data[i: i + input_window + output_window]  # 取位置i开始的长度为input_window+output_window的数据片段 (in+out, ...)
        x.append(a[0: input_window])  # 将片段的前input_window个数据作为输入添加到x中  (in, ...)
        y.append(a[input_window: input_window + output_window])  # 将片段的后output_window个数据作为输出添加到y中  (out, ...)
