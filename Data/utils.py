...
def get_dataset(config):
    ...
    """
    if config["task"] == "multi_step":  # 多步任务
        try:
            '''动态导入指定模块并且实例化对应的数据集'''
            return getattr(importlib.import_module('libcity.data.dataset.multi_step_dataset'),config['dataset_class'])(config)
        except AttributeError:
            raise AttributeError('dataset_class is not found')
    elif config["task"] == "single_step":  # 单步任务
        try:
            return getattr(importlib.import_module('libcity.data.dataset.single_step_dataset'),config['dataset_class'])(config)
        except AttributeError:
            raise AttributeError('dataset_class is not found')
